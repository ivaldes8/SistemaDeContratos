    ...
    <div class="justify-content-center">
        ...
        @endif
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{url('contratos_especificos/'.$contratoE->id)}}" method="POST">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="col-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="col-8" style="border: solid 1px;" id="objeto" name="noContrato">
                                                    <option value = '{{$contratoE->CMs->id}}'>{{$contratoE->CMs->noContrato}}</option>
                                            </select>
                                            <p class="mt-3">Objeto del Contrato: {{$contratoE->CMs->objeto}}</p>
                                            <p>Organismo: {{$contratoE->CMs->organismos->nombreO}}</p>
                                            <p>Entidad: {{$contratoE->CMs->cliente->nombre}}</p>
                                            <p>No.Contrato Específico: Autogenerable</p>
                                        </div>
                                        <div class="form-group">
                                            <label>Observaciones:</label>
                                            <textarea name="observaciones" class="form-control" placeholder="{{$contratoE->observaciones}}"  value="{{$contratoE->observaciones}}" id="" cols="15" rows="5"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <div class='input-group'>
                                                <input type='text' class="@error('fechaIni') is-invalid @enderror form-control" id="datepicker" value="{{$contratoE->fechaIni}}"  name="fechaIni" />
                                            </div>
                                        </div>
                                        <div class="form-group mb-3">
                                            <label>Fecha de vencimiento:</label>
                                            <div class='input-group'>
                                                <input type='text' class="@error('fechaEnd') is-invalid @enderror form-control" id="datepicker2" value="{{$contratoE->fechaEnd}}"  name="fechaEnd" />
                                            </div>
                                        </div>
                                        <div class="form-group mb-3">
                                            <label>Nombre del Prestador:</label>
                                            <input type="text" class="@error('ejecutorName') is-invalid @enderror form-control" value="{{$contratoE->ejecutorName}}" name="ejecutorName">
                                        </div>
                                        <div class="form-group mb-3">
                                            <label>Nombre del Cliente:</label>
                                            <input type="text" class="@error('clienteName') is-invalid @enderror form-control" value="{{$contratoE->clienteName}}" name="clienteName">
                                        </div>
                                        <div class="form-group">
                                            <label>Monto:</label>
                                            <input type="number"  step="0.001" class="@error('monto') is-invalid @enderror form-control" value="{{$contratoE->monto}}"  name="monto">
                                        </div>
                                        <div class="form-group mb-3">
                                            <label for="">Área:</label>
                                            <select class="col-8" style="border: solid 1px;" name="area">
                                            @foreach ( $area as $item )
                                                <option value = '{{$item->idarea}}' {{$contratoE->idArea == $item->idarea ? 'selected' : ''}}>{{$item->Expr4}}</option>
                                            @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select name="estado" id="estado" class="col-8">
                                                <option {{$contratoE->estado == 'Sin Comenzar' ? 'selected' : ''}}>Sin Comenzar</option>
                                                <option {{$contratoE->estado == 'En Proceso' ? 'selected' : ''}}>En Proceso</option>
                                                <option {{$contratoE->estado == 'Terminado' ? 'selected' : ''}}>Terminado</option>
                                                <option {{$contratoE->estado == 'Cancelado' ? 'selected' : ''}}>Cancelado</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                  ...
                                  <div class="col-12">
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><input type="checkbox" name = "services[]" checked value = "{{$item->idServicio}}"></td>
