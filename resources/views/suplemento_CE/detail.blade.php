                ...
                <div class="card-header">
                    <div class="row">
                        <div class="col-8" style="font-size: 80%">
                            <p style="margin-bottom: 0px">No. Contrato Específico: {{$CE[0]->noContratoEspecifico}}</p>
                            <p style="margin-bottom: 0px">Área del Contrato Específico: {{$CE[0]->areas->Expr4}}</p>
                            <p style="margin-bottom: 0px">Monto: {{$CE[0]->monto}}</p> 
                        </div>
                        <div class="col-4">
                            <a href="{{url('suplementoce/create/' .$CE[0]->idCEspecifico)}}" class="btn btn-primary mt-3">Crear Suplemento de Contrato Específico</a>
                        </div>
                        <hr/>
                    </div>
                    ...
                    <table class="table table-bordered table-sm table-striped" id="mytable">
                        <thead>
                            <tr style="font-size: 70%;">
                                <th style="width: 20px;">No.ContratoEspecífico</th>
                        ...
                        <tr style="font-size: 70%; {{\Carbon\Carbon::createFromFormat('d-m-y', \Carbon\Carbon::parse($item->fechaEndSup)->format('d-m-y'))->lte($now) ? 'background-color: red' : ''}}"
                            class = "{{ \Carbon\Carbon::createFromFormat('d-m-y', \Carbon\Carbon::parse($item->fechaEndSup)->format('d-m-y'))->gte($now) 
                                    ...
                                    \Carbon\Carbon::createFromFormat('d-m-y', \Carbon\Carbon::parse($item->fechaEndSup)->format('d-m-y'))->lte($ThreeDaysearly) 
                                        ? 'bg-warning' : ''}}">
                            <td>{{$item->CEs->noContratoEspecifico}}</td>
                            ...
                            <td>
                                <form action="{{url('suplementoce/'.$item->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
